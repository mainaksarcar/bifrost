...
test.describe('Copilot device login', () => {
  ...
  test('should persist the key and rediscover models once the grant completes', async ({ page, providersPage }) => {
    ...
    // The saved token belongs to this client ID and editing it discards the
    // token, so the field stays locked until re-authentication is chosen.
    await expect(providersPage.copilotClientIdInput).toBeDisabled()
    await providersPage.copilotReauthToggle.click()
    await expect(providersPage.copilotClientIdInput).toBeEnabled()
    await providersPage.copilotReauthToggle.click()
    await expect(providersPage.copilotClientIdInput).toBeDisabled()

    // Visiting the other method and coming back is navigation, not a decision
    // to sign out, so the credential and the client ID behind it must survive.
    await providersPage.copilotManualTokenTab.click()
    await expect(providersPage.copilotApiTokenInput).toBeVisible()
    await providersPage.copilotDeviceLoginTab.click()
    await expect(providersPage.copilotAuthStatusCard).toBeVisible()
    await expect(providersPage.copilotClientIdInput).toHaveValue(CLIENT_ID)
    await expect(providersPage.copilotClientIdInput).toBeDisabled()

    // Editing the client ID clears the token minted for it, so cancelling has to
    // restore both rather than leave the key without a credential. The card stays
    // up throughout, since Cancel lives inside it.
    await providersPage.copilotReauthToggle.click()
    await providersPage.copilotClientIdInput.fill('Ov23liSomethingElse')
    await expect(providersPage.copilotReauthToggle).toBeVisible()
    await providersPage.copilotReauthToggle.click()
    await expect(providersPage.copilotClientIdInput).toHaveValue(CLIENT_ID)
    await expect(providersPage.copilotClientIdInput).toBeDisabled()
  ...
  test('should not report the token as missing before there was a chance to supply it', async ({ providersPage }) => {
    await providersPage.selectProvider(PROVIDER)
    await providersPage.addKeyBtn.click()
    await expect(providersPage.keyForm).toBeVisible()

    // The form validates on change and the save button subscribes to validity,
    // so anything that clears key.value with shouldValidate surfaces this
    // against a field the operator has not been offered yet.
    const missingToken = providersPage.keyForm.getByText('API Key is required')

    // Editing the client ID clears any token minted for the previous one.
    await providersPage.copilotDeviceLoginTab.click()
    await providersPage.copilotClientIdInput.fill(CLIENT_ID)
    await expect(missingToken).toHaveCount(0)

    // Choosing an auth method clears it too, and is the first point at which
    // the token field is actually rendered.
    await providersPage.copilotManualTokenTab.click()
    await expect(providersPage.copilotApiTokenInput).toBeVisible()
    await expect(providersPage.copilotApiTokenInput).toHaveValue('')
    await expect(missingToken).toHaveCount(0)

    // The field naming the credential must agree with the tab that leads to
    // it, which it did not while the two were maintained separately.
    await expect(providersPage.keyForm.getByText('Copilot API Token', { exact: true })).toHaveCount(0)
  })
})
