...
// and returns the user prompt plus the system message to apply.
func (p *githubCopilotProvider) prepareSDKChat(request *schemas.BifrostChatRequest) (string, string, *schemas.BifrostError) {
	if request == nil || request.Model == "" || len(request.Input) < 1 || len(request.Input) > 2 || len(request.RawRequestBody) > 0 {
		return "", "", configurationError("github copilot: SDK mode currently accepts one text user message and an optional system message")
	}
	if request.Params != nil {
		if unsupported := contractChangingParams(request.Params); len(unsupported) > 0 {
			return "", "", configurationError("github copilot: " + strings.Join(unsupported, ", ") + " not supported in SDK mode")
		}
	...
	return *request.Input[len(request.Input)-1].Content.ContentStr, system, nil
}
...
func (p *githubCopilotProvider) sdkChatCompletion(ctx *schemas.BifrostContext, key schemas.Key, request *schemas.BifrostChatRequest) (*schemas.BifrostChatResponse, *schemas.BifrostError) {
	prompt, system, bErr := p.prepareSDKChat(request)
	if bErr != nil {
		return nil, bErr
	}
	started := time.Now()
...
func (p *githubCopilotProvider) sdkChatCompletionStream(ctx *schemas.BifrostContext, postHookRunner schemas.PostHookRunner, postHookSpanFinalizer func(context.Context), key schemas.Key, request *schemas.BifrostChatRequest) (chan *schemas.BifrostStreamChunk, *schemas.BifrostError) {
	...
	go func() {
		...
		providerUtils.ProcessAndSendResponse(ctx, postHookRunner, providerUtils.GetBifrostResponseForStreamResponse(nil, final, nil, nil, nil, nil), responseChan, postHookSpanFinalizer)
	}()
